    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (count <= 1)
            {
                ...
                channel = new TcpChannel(61001);
                ChannelServices.RegisterChannel(channel);

                // Create a single marshaled object
                SwitchServiceClass remServe = new SwitchServiceClass();

                ObjRef obj = RemotingServices.Marshal(remServe, "TcpService");

                // Create apllications MainForm
                Switch frmMain = new Switch();

                // provide marshaled object with reference to Application
                remServe.theMainForm = (SwitchServerApplication.ISwitchRemote)frmMain;

                Application.Run(frmMain);

                // Application closed...
                RemotingServices.Unmarshal(obj);
                RemotingServices.Disconnect(remServe);
            }
            else
            {
